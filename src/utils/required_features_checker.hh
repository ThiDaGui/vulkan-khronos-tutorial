...
{
    vk::StructureChain<ChainElements...> required_features_;

public:
    explicit RequiredFeaturesChecker(const ChainElements&... elems)
        : required_features_(vk::StructureChain<ChainElements...>(elems...))
    {}

private:
    [[nodiscard]] static bool checkFeaturesInternal(
    ...
    template <typename T>
    [[nodiscard]] bool checkSupported(const vk::StructureChain<ChainElements...>& supported_features) const
    {
        const T required_features_struct = required_features_.template get<T>();
        const T supported_features_struct = supported_features.template get<T>();

        return checkFeaturesInternal(required_features_struct, supported_features_struct);
    }

    template <typename T1, typename T2, typename... Ts>
    [[nodiscard]] bool checkSupported(const vk::StructureChain<ChainElements...>& supported_features) const
    {
        return checkSupported<T1>(supported_features) &&
               checkSupported<T2, Ts...>(supported_features);
    }

public:
    [[nodiscard]] bool checkSupported(const vk::PhysicalDevice& physical_device) const
    {
        vk::StructureChain<ChainElements...> supported_features = physical_device.getFeatures2<ChainElements...>();
        return checkSupported<ChainElements...>(supported_features);
    }

    template <typename T = std::tuple_element_t<0, std::tuple<ChainElements...>>, size_t Which = 0>
    [[nodiscard]] T& get() & noexcept
    {
        return required_features_.template get<T, Which>();
    }
